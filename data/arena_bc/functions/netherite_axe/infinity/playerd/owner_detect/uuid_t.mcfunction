tag @e[sort=nearest,type=!player,tag=!my_arrow_playerd,tag=!not_my_arrow_playerd,limit=1] add uuid_detect_playerd
...
execute as @e[tag=uuid_detect_playerd] at @s if score #playerd uuid_detect matches 4 run function arena_bc:netherite_axe/infinity/playerd/owner_detect/my_arrow
execute as @e[tag=uuid_detect_playerd] at @s unless score #playerd uuid_detect matches 4 run function arena_bc:netherite_axe/infinity/playerd/owner_detect/not_my_arrow