execute if entity @e[type=item,nbt={Item:{tag:{power_bow:1b}}},distance=..2] run function arena_bc:surface_inner_bow/playerb/flame_bow
execute if entity @e[type=item,nbt={Item:{tag:{flame_bow:1b}}},distance=..2] if score @s drop_bow matches 1 run function arena_bc:surface_inner_bow/playerb/power_bow
