kill @e[type=trident,tag=my_trident_playerc]
execute store success score #playerc drop_trident run kill @e[type=item,nbt={Item:{tag:{lightning:1b}}}]
...
execute if score #playerc drop_trident matches 0 run playsound item.trident.return player @a
