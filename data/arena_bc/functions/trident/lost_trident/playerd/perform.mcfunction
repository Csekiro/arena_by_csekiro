kill @e[type=trident,tag=my_trident_playerd]
execute store success score #playerd drop_trident run kill @e[type=item,nbt={Item:{tag:{lightning:1b}}}]
...
execute if score #playerd drop_trident matches 0 run playsound item.trident.return player @a
