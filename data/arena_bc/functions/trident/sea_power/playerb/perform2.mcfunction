execute as @a[tag=playerb,limit=1] at @s run kill @e[type=item,distance=..3]
execute as @a[tag=playerb,limit=1] at @s run summon minecraft:lightning_bolt ~ ~-2 ~
scoreboard players remove #sea_power lightning_times 1
execute if score #sea_power lightning_times matches 0 run schedule function arena_bc:trident/sea_power/playerb/back_survival_t 26t append
...
execute unless score #sea_power lightning_times matches 0 run schedule function arena_bc:trident/sea_power/playerb/perform2 1t append